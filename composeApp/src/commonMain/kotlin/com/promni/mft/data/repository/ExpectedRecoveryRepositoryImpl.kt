...
class ExpectedRecoveryRepositoryImpl(
    ...
    override suspend fun setExpectedRecovery(muscleId: Long, time: Long) {
        val now = SystemTime.nowMillis()
        expectedRecoveryDao.upsert(ExpectedRecoveryEntity(muscleId, time, now))
    }

    override suspend fun getExpectedRecovery(id: Long) = expectedRecoveryDao.item(id)

    override suspend fun clearExpectedRecovery(muscleId: Long) = expectedRecoveryDao.delete(muscleId)
}
